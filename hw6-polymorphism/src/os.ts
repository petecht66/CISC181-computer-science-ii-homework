...
export class OperatingSystem {
    ...
    openFile(title: string): BasicFile {
        for (let file of this.files) {
            if (file.getName() === title) {
                return file;
            }
        }
        return new BasicFile(title, "");
    }
...
export function makeTestOS(): OperatingSystem {
    let operating_system: OperatingSystem = new OperatingSystem();
    operating_system.createFile(new BasicFile("first.txt", "Hello, world!"));
    operating_system.createFile(new EditableFile("second.txt", "Hola, mundo!"));
    operating_system.createFile(
