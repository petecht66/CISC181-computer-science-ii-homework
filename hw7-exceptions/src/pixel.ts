...
export class Pixel {
    ...
    getRGB(): string {
        if (
            !this.inRange(this.red) ||
            !this.inRange(this.green) ||
            !this.inRange(this.blue)
        ) {
            throw new Error(
                "Invalid RGB values: " +
                    this.red +
...
export function makeColor(color: string): Pixel {
    if (color == "red") {
        let pixel: Pixel = new Pixel(255, 0, 0);
        return pixel;
    }
    if (color == "green") {
        let pixel: Pixel = new Pixel(0, 255, 0);
        return pixel;
    }
    if (color == "blue") {
        let pixel: Pixel = new Pixel(0, 0, 255);
        return pixel;
    } else {
