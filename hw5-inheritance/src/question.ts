...
export class QuizQuestion {
    ...
    check(input: string): boolean {
        return input === this.expectedAnswer;
    }
}
...
export class TrueFalseQuestion extends QuizQuestion {
    ...
    override check(input: string): boolean {
        return input.toLowerCase() === this.expectedAnswer.toLowerCase();
    }
}
...
export class MultipleChoiceQuestion extends QuizQuestion {
    ...
    ask(): string {
        let answer_string: string = this.body + "\n";
        for (let i: number = 0; i < this.options.length; i++) {
            let index_string: string = (i + 1).toString();
            answer_string += index_string + ". " + this.options[i] + "\n";
        }
        answer_string += "> ";
