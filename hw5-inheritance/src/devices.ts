...
export class Phone extends Device {
    ...
    constructor(name: string, weight: number) {
        ...
        this.operatingSystem.addFile("contacts.txt", "Ada\nBabbage\nCaptain");
        this.operatingSystem.addFile("history.log", "");
    }

    addContact(contact: string) {
        let contactsFile = this.operatingSystem.openFile("contacts.txt");
        contactsFile.append("\n" + contact);
    }

    getHistory(): string {
        let historyFile = this.operatingSystem.openFile("history.log");
        return historyFile.getContents();
    }
    ...
    sendText(contact: string, message: string): boolean {
        ...
        let historyFile = this.operatingSystem.openFile("history.log");
        if (
            !contactsFile.getContents().includes(contact) ||
            historyFile === new ColorfulFile("EMPTY", "")
        ) {
            return false;
        }
        let text: string = "Sent text to " + contact + ": " + message + "\n";
        historyFile.append(text);
        return true;
    }
}
