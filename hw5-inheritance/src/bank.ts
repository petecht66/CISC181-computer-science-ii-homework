...
export class SafeAccount extends BankAccount {
    ...
    override checkPassword(givenPassword: string): boolean {
        return givenPassword === this.password && this.password !== "";
    }

    public log: string[] = [];

    override deposit(amount: number): number {
        let amount_str: string = amount.toString();
        if (amount < 0) {
            this.log.push("Cannot deposit negative amount " + amount_str);
        }
        if (amount > 0) {
            this.balance += amount;
            this.log.push("Deposited " + amount_str);
        }
        return this.balance;
    }
    ...
    override withdraw(amount: number): number {
        let amount_str: string = amount.toString();
        if (amount < 0) {
            this.log.push("Cannot withdraw negative amount " + amount_str);
            return 0;
        }
        ...
        if (amount <= this.balance && amount > 0) {
            this.balance -= amount;
            this.log.push("Withdrew " + amount_str);
            return amount;
        } else {
            return 0;
        }
...
export function safeAccountExample() {
    const mySafeAccount = new SafeAccount(100);
    console.log("Changing my password");
    mySafeAccount.changePassword("123456789");
    console.log(
        "Checking the old password",
        mySafeAccount.checkPassword("1234"),
    );
    console.log(
        "Checking the new password",
        mySafeAccount.checkPassword("123456789"),
    );
    console.log(
        "Depositing 50 dollars. New balance:",
